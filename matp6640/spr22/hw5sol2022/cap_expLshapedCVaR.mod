...
param cost_bound;  #  maximum required expected costs
param alpha >= 0;  #  probability of the tail
...
var vCVaR{SCENARIOS} >= 0;  #  excess cost above mu
...
+ 0.1*mu + 0.1*(1/alpha)*sum{s in SCENARIOS} prob[s] * vCVaR[s];
...
subject to cuts_vCVaR{s in SCENARIOS, k in 1..ncuts[s]}:
  mu + vCVaR[s] >= theta[s];
  
#subject to expupperbound: sum{(i,j) in EDGES} capexpcost[i,j]*x[i,j]
#   + sum{s in SCENARIOS} prob[s]*theta[s] <= cost_bound;
