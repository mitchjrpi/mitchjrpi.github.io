% randomly generate a maxcut instance on a square torus,
% with edge weights 1 or -1
%
% order the entries in A in left to right then top to bottom in the grid
...
for i=1:gsize-1
    for j=1:gsize-1
        k=(i-1)*gsize+j;
        A(k,k+1) = 2*floor(2*rand)-1;
        A(k,k+gsize) = 2*floor(2*rand)-1;
    end
    A((i-1)*gsize+1,i*gsize) = 2*floor(2*rand)-1;
    A(i*gsize,(i+1)*gsize) = 2*floor(2*rand)-1;
end

for j=1:gsize-1
    A(j,gsize*(gsize-1)+j) = 2*floor(2*rand)-1;
    A(gsize*(gsize-1)+j,gsize*(gsize-1)+j+1) = 2*floor(2*rand)-1;
end

A(gsize,gsize*gsize) = 2*floor(2*rand)-1;
A(gsize*(gsize)-1+1,gsize*gsize) = 2*floor(2*rand)-1;
