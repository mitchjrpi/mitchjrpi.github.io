...
c    PAGES   =  {493--513}   }
c
c  This routine ensures that
c    the number of +1's and the number of -1's are identical.
c    (20 October 1997.)
c
c
      IMPLICIT NONE
...
c
c  Ensure TOTALC is zero.
c

      K=0
      DO 91 I=1,10*NVERT
	IF (ABS(TOTALC) .LT. 0.5) GOTO 92
	J=1+2*DRAND()*NVERT
	IF (TOTALC .GT. 0 .AND. C(J) .GT. 0) THEN
	  C(J) = -1
	  TOTALC = TOTALC-2.D0
	  K=K+1
          GOTO 91
        ENDIF
	IF (TOTALC .LT. 0 .AND. C(J) .LT. 0) THEN
	  C(J) = 1
	  TOTALC = TOTALC+2.D0
	  K=K+1
	  GOTO 91
