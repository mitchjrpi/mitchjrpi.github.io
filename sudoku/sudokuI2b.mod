...
param GRID {ROW,COLUMN} >= 0;

var x{NUMBER, ROW, COLUMN} binary;  # >= 0, <= 1;

minimize total : sum {n in NUMBER, i in ROW, j in COLUMN} n*i*j*x[n,i,j];
...
subject to fixed{i in ROW, j in COLUMN : GRID[i,j] > 0}:
  x[GRID[i,j],i,j] = 1;
